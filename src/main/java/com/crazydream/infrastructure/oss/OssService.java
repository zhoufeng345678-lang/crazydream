...
public class OssService {
     ...
     * @param file 上传的文件
     * @param userId 用户ID
     * @param directory 存储目录（如 "documents"）
     * @return OSS文件URL
     * @throws IllegalArgumentException 如果文件格式不支持或文件过大
     * @throws RuntimeException 如果上传失败
     */
    public String uploadDocument(MultipartFile file, Long userId, String directory) {
        // 验证文件不为空
        if (file == null || file.isEmpty()) {
            throw new IllegalArgumentException("文件不能为空");
        }
        ...
        // 生成唯一文件名
        String fileName = generateFileName(userId, extension);
        String objectKey = directory + "/" + userId + "/" + fileName;
        ...
        try (InputStream inputStream = file.getInputStream()) {
            ...
            // 返回完整的访问URL（带https协议）
            String fileUrl = "https://" + ossConfig.getDomain() + "/" + objectKey;
            logger.info("文档上传成功: userId={}, objectKey={}, url={}", userId, objectKey, fileUrl);
